...
class Patient extends CI_Controller
{
    ...
    public function legacyreport($patient_number){
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $data['title'] = "Medstation | Legacy Medical Report";
            $data['search_url'] = "index.php/patient/legacyreport";
            ...
            $this->load->view('templates/header', $data);
            $this->load->view('templates/mainheader', $data);
            $this->load->view('frontdesk/patient/legacyreport', $data);
            $this->load->view('templates/footer');
    ...
    public function viewall($patient_number){
        if ($this->session->userdata('logged_in')) {
            $this->confirmUrl();
			...
			$data['admissiondetail'] = $this->patient_model->getAdmissionDetails($patient_number);
			$data['financialdetail'] = $this->bills_model->getFinancialRecords($patient_number);
			$data['nurseaction'] = $this->patient_model->getNurseActions($patient_number);
			
			$data['unadjusteddeposit'] = $this->bills_model->getUnadjusteddeposit($patient_number);
			$data['unpaidbills'] = $this->bills_model->getUnpaidbills($patient_number);
            ...
            $data['doctors'] = $this->staff_master_model->getDoctors();
			$data['staffs'] = $this->staff_master_model->getStaff();
            ...
            $data['patient_types'] = $this->patient_type_model->getType();
			$data['providers'] = $this->hmo_model->getHmo();
            $data['title'] = "Medstation | Veiw All Previous Visit";
            $data['search_url'] = "index.php/patient/viewall";
            ...
            $this->load->view('templates/header', $data);
            $this->load->view('templates/mainheader', $data);
            $this->load->view('frontdesk/patient/viewall', $data);
            $this->load->view('templates/footer');
    ...
    public function queueCheckIn()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            if (!isset($moduleAccess) || $moduleAccess['access'] != 'W') {
				...
				/*$checkIfLock = $this->patient_model->checkIfLocked($patient_number);
				//$checkIfLock = 'Y';
				if ($checkIfLock == 'Y') {
					log_message("debug", "Patient is locked");
					$status = array("STATUS" => "false", "ERROR" => "Patient file is Locked");
	...
	public function changeFIleType()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $patienDetails = $this->patient_model->getPatient($this->input->post('patient_number'));
			...
			//$data["patient_type_code"] = $this->input->post('patient_type');
			if($patienDetails['patient_type_code']=='H'){
				$data["hmo_code"] = null;
