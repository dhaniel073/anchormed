...
class Billing extends CI_Controller
{
    ...
    public function viewBill($reference,$checkvalue=1)
    {
        if ($this->session->userdata('logged_in')) {
            $this->moduleAccessCheck(VIEW_BILLS);
			...
			$refpaycount = $this->bills_model->getBillPartialByReferenceCount($reference);
			
			if($refpaycount['total_count'] > 0){
				$data['partial_current_bill'] = $this->bills_model->getBillPartialByReference($reference);
				$data['partial_current_bill'] = $data['partial_current_bill'][0];
			}
			...
			$data['referenceDate'] = $this->bills_model->getBillDate($reference);		
            $data['patient'] = $this->patient_model->getPatient($patient_number);
            $data['state'] = $this->state_model->getState($data['patient']['address_state_code']);
            $data['country'] = $this->country_model->getCountries($data['patient']['address_country_code']);
    ...
    public function partialPaymentUpdate($reference_id,$checkvalue=1)
    {
        ...
        $data['state'] = $this->state_model->getState($data['patient']['address_state_code']);
        $data['country'] = $this->country_model->getCountries($data['patient']['address_country_code']);
		$data['patient_deposit'] = $this->patient_deposit_model->getLatestDepositByPatientNumber($partial_payment_bill[0]["patient_number"]);
		...
		$data['hmo_name'] = $this->hmo_model->getHmo($data['patient']['hmo_code']);
		$data['checkvalue'] = $checkvalue;
		$data['referenceDate'] = $this->bills_model->getBillDate($reference_id);
    ...
    public function updateCustomItemPrice(){
		...
		if($this->input->post("custom_quantity") > 0 ){

            $updatedQuantity = $this->input->post("custom_quantity");

        }

        $data['selling_price'] = $updatedTotal;
		$data['qty'] = $updatedQuantity;
    ...
    public function nonCustomerBill($reference, $checkvalue=1)
    {
        $this->utilities->aunthenticateAccess(CREATE_BILL, "home");
        ...
        $data['departments'] = $this->department_model->getDepartments();
		if(isset($data['patient']['hmo_code'])){$hmoname_v=$this->hmo_model->getHmo($data['patient']['hmo_code']);}else{$hmoname_v="";}
		$data['hmo_name'] = $hmoname_v;
		//$data['hmo_name'] = $this->hmo_model->getHmo($data['patient']['hmo_code']);
    ...
    public function currentBill($patient_number,$checkvalue=1)
    {
