...
class Bills_model extends CI_Model
{
    ...
    public function gettotalamount($reference_id){
       
        $query = $this->db->select('sum(ifnull(selling_price, (unit_price*qty))) as total_amt')->from('bills')->where('reference_id', $reference_id)->get();
        return $query->row()->total_amt;
    }

    public function gettotalamountdue($reference_id){
       
        $query = $this->db->select('sum(ifnull(amount_paid, (original_bill_amount - amount_paid))) as total_amt')->from('patient_partial_payments')->where('reference_id', $reference_id)->get();

        // $query = $this->db->select('amount_paid')->from('patient_partial_payments')->where('reference_id', $reference_id)->get();
        // $query = $this->db->get_where('patient_partial_payments', array('reference_id' => $reference_id));
        // $total_paid =  $query->row()->tot_amount;
	 ...
	 public function getFinancialRecords($patient_number)
    {
		...
		/*$this->db->select("reference_id, sum(if(selling_price>0,selling_price,(unit_price*qty))) as total_price, status,date_created");
        $this->db->group_by("reference_id,status");
        //$where_clause = ("'patient_number' = $patient_number");
        $this->db->where("'patient_number' = $patient_number");
        $this->db->from('bills');
        $query = $this->db->get();*/
		
		$query = $this->db->select('reference_id, sum(if(selling_price>0,selling_price,(unit_price*qty))) as total_price, status,date_created')->from('bills')->where('patient_number', $patient_number)->group_by('reference_id,status')->get();
        return $query->result_array();
    }
	
	
	public function getUnpaidbills($patient_number)
    {
        
		

		$sql = "select IFNULL(sum(case when selling_price is null then unit_price*qty else selling_price end),0) as unposted_bill from bills where status in ('R','N') and patient_number='$patient_number'";
		$query = $this->db->query($sql);
		$unpostedbill = $query->row()->unposted_bill;
		
		//$query = $this->db->select('IFNULL(sum(amount_paid),0) as total_amount')->from('patient_payment_mode')->where('patient_number', $patient_number)->where('service_name != ','Deposit')->get();
        //$postedbill = $query->row()->total_amount;
		
		
		$sql = "select IFNULL(sum(amount_paid),0) as total_amount from patient_payment_mode where patient_number='$patient_number'";
		$query = $this->db->query($sql);
		$postedbill = $query->row()->total_amount;
		...
		if($unpostedbill==0){
			...
			$diffunpaid = $unpostedbill - $postedbill;
		}
    ...
    public function getUnadjusteddeposit($patient_number)
    {

		$query = $this->db->select('IFNULL(sum(deposit_amount),0) as total_amount')->from('patient_deposit')->where('patient_number', $patient_number)->get();
