...
class Server{
	...
	//	$config = $db['oauth'];
		...
		$database = $this->medstation->db->database;
		$host = $this->medstation->db->hostname;
		$dsn      = "mysql:dbname=$database;host=$host";
		$username = $this->medstation->db->username;
		$password = $this->medstation->db->password;
	...
	*/
	public function password_credentials(){
		...
		$grantType = new OAuth2\GrantType\UserCredentials(new OAuth2\CustomUserSource());
		// $grantType = new OAuth2\CustomUserSource();
